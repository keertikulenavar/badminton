...
/* Auto-create cancelled_bookings table (same table admin uses) */
$conn->query("CREATE TABLE IF NOT EXISTS cancelled_bookings (booking_id INT PRIMARY KEY)");

/* ── Soft-cancel: insert into cancelled_bookings instead of deleting ── */
if(isset($_GET['cancel_id'])){
    $bid = (int)$_GET['cancel_id'];
    /* Make sure this booking belongs to this user */
    $check = $conn->query("SELECT booking_id FROM bookings WHERE booking_id='$bid' AND user_email='$user_email'");
...
/* ── Fetch all bookings + cancelled status ── */
$sql = "
...
if($result && $result->num_rows > 0){
    while($row = $result->fetch_assoc()){
        $rows[] = $row;
        if(!empty($row['is_cancelled']))         $cancelled++;
        elseif($row['booking_date'] >= $today)   $upcoming++;
        else                                      $completed++;
    }
}
$total = count($rows);
...
tbody tr.row-cancelled{ opacity:0.8; }
...
.time-badge{
    ...
    border-radius:6px; padding:4px 11px; font-size:13px; font-weight:bold;
}

/* Status badges */
.status-upcoming  { display:inline-flex;align-items:center;gap:5px;background:#e8f5e9;color:#27500a;border-radius:20px;padding:4px 10px;font-size:12px;font-weight:bold; }
.status-past      { display:inline-flex;align-items:center;gap:5px;background:#f5f5f5;color:#555;border-radius:20px;padding:4px 10px;font-size:12px; }
.status-cancelled { display:inline-flex;align-items:center;gap:5px;background:#ffebee;color:#b71c1c;border-radius:20px;padding:4px 10px;font-size:12px;font-weight:bold; }
...
.cancel-btn{
    ...
    border:1px solid #ffcdd2; padding:6px 13px; border-radius:7px;
    font-size:12px; font-weight:bold; text-decoration:none; transition:0.2s; cursor:pointer;
}
...
tfoot td{ padding:12px 14px; font-size:13px; color:#888; border-top:2px solid #e8f5e9; }
...
<div class="container">
    ...
    <div class="summary-bar">
        ...
        <div class="sum-cell">
            ...
            <div class="val red"><?php echo $cancelled; ?></div>
        </div>
    </div>
    ...
    <div class="table-wrap">
        ...
        <div class="search-row">
            <input type="text" id="searchInput" placeholder="🔍  Search by date or time...">
        </div>
        ...
        <table id="bookTable">
            ...
            <tbody>
                <?php foreach($rows as $i => $row):
                    $isCancelled  = !empty($row['is_cancelled']);
                    $isUpcoming   = !$isCancelled && ($row['booking_date'] >= $today);
                    $display_date = date('M d, Y', strtotime($row['booking_date']));
                    $day_name     = date('l', strtotime($row['booking_date']));
                ?>
                <tr class="<?php echo $isCancelled ? 'row-cancelled' : ''; ?>" style="animation-delay:<?php echo $i * 0.05; ?>s">
                    <td>
                        <div class="date-main"><?php echo $display_date; ?></div>
                        <div class="date-sub"><?php echo $day_name; ?></div>
                    </td>
                    <td>
                        <span class="time-badge"><?php echo htmlspecialchars($row['booking_time']); ?></span>
                    </td>
                    <td>
                        <?php if($isCancelled): ?>
                    ...
                    <td>
                        ...
                        <?php elseif($isUpcoming): ?>
                            <a class="cancel-btn"
                               href="history.php?cancel_id=<?php echo $row['booking_id']; ?>"
                               onclick="return confirm('Cancel booking on <?php echo $display_date; ?> at <?php echo htmlspecialchars($row['booking_time']); ?>?')">
                               ✕ Cancel
                            </a>
                        <?php else: ?>
            ...
            <tfoot>
                <tr>
                    <td colspan="4" id="rowCount">Showing <?php echo $total; ?> of <?php echo $total; ?> bookings</td>
                </tr>
            </tfoot>
        </table>
